...
let allEtfs = [];
let basketSel = new Set();   // tickers selected for basket builder (F1)
let pfChartNav = null;
...
async function loadOverview() {
    try {
        ...
        const params = new URLSearchParams({ period: currentPeriod, mode });
        if (mode === 'full') {
...
function getQualityParams() {
    return {
        ...
        min_div_payments: parseInt(document.getElementById('q-min-payments')?.value) || 0,
        max_nav_erosion_pct: parseFloat(document.getElementById('q-max-nav-eat')?.value) || 100,
    };
}

/** Fetch ETF data respecting universe mode toggle */
async function fetchEtfsWithMode(sortBy, sortDir, limit) {
    const mode = getUniverseMode();
    ...
    if (mode === 'full') {
        ...
        const q = getQualityParams();
        for (const [k, v] of Object.entries(q)) params.set(k, v);
        const resp = await fetch(`${API}/universe?${params}`);
        const data = await resp.json();
        return data.etfs;
    } else {
        const resp = await fetch(`${API}/etfs?${params}`);
...
function renderTable() {
    ...
    tbody.innerHTML = etfs.map(e => `
        <tr>
            <td><input type="checkbox" class="basket-cb" value="${e.ticker}" ${basketSel.has(e.ticker) ? 'checked' : ''}></td>
            <td><strong>${e.ticker}</strong>${e.real_yield != null && e.real_yield < 0 ? ' <span title="Yield trap: NAV erosion exceeds distribution — you lost money" style="color:var(--red);font-weight:bold">⚠</span>' : ''}</td>
            ...
            <td class="yield-col">${pct(e.after_tax_yield)}</td>
            ...
            <td class="${e.real_yield != null && e.real_yield >= 0 ? 'positive' : 'negative'}">${pct(e.real_yield)}</td>
            <td class="${e.net_real_yield != null && e.net_real_yield >= 0 ? 'positive' : 'negative'}">${pct(e.net_real_yield)}</td>
            <td class="${e.quality_score != null ? (e.quality_score >= 60 ? 'positive' : e.quality_score >= 40 ? '' : 'negative') : ''}">${e.quality_score != null ? e.quality_score.toFixed(0) : '--'}</td>
    ...
    // Basket checkbox toggles (F1)
    tbody.querySelectorAll('.basket-cb').forEach(cb => {
        cb.onchange = () => {
            if (cb.checked) basketSel.add(cb.value);
            else basketSel.delete(cb.value);
            const cnt = document.getElementById('basket-count');
            if (cnt) cnt.textContent = basketSel.size;
        };
    });
}

async function buildBasket() {
    const el = document.getElementById('basket-result');
    if (!el) return;
    if (basketSel.size === 0) {
    ...
    try {
        ...
        const data = await resp.json();
        if (!data.blended) { el.innerHTML = '<p>No data for selected tickers.</p>'; return; }
        const b = data.blended;
        const trapNote = b.trap_count > 0
            ? `<div class="basket-warn">⚠ ${b.trap_count} holding(s) flagged as yield traps (NAV erosion &gt; distribution, or dividend cuts).</div>` : '';
        const corrNote = b.avg_correlation_sp500 != null && b.avg_correlation_sp500 > 0.7
            ? `<div class="basket-warn">⚠ High avg correlation (${(b.avg_correlation_sp500*100).toFixed(0)}%) — limited diversification.</div>` : '';
        el.innerHTML = `
            <div class="basket-summary">
                <div class="bs-row"><span>Blended Yield</span><strong>${pct(b.current_yield)}</strong></div>
                <div class="bs-row"><span>After-Tax Yield</span><strong>${pct(b.after_tax_yield)}</strong></div>
                <div class="bs-row"><span>Real Yield (Yld+NAV)</span><strong class="${b.real_yield>=0?'positive':'negative'}">${pct(b.real_yield)}</strong></div>
                <div class="bs-row"><span>Net Real Yield (−exp)</span><strong class="${b.net_real_yield>=0?'positive':'negative'}">${pct(b.net_real_yield)}</strong></div>
                <div class="bs-row"><span>Quality Score</span><strong>${b.quality_score != null ? b.quality_score.toFixed(0) : '--'}</strong></div>
                <div class="bs-row"><span>Avg Sharpe</span><strong>${fmt(b.avg_sharpe)}</strong></div>
                <div class="bs-row"><span>Avg Expense</span><strong>${b.avg_expense != null ? b.avg_expense.toFixed(2)+'%' : '--'}</strong></div>
                <div class="bs-row"><span>Avg Correlation (S&amp;P)</span><strong>${b.avg_correlation_sp500 != null ? b.avg_correlation_sp500.toFixed(2) : '--'}</strong></div>
                <div class="bs-row"><span>Avg Max Drawdown</span><strong class="negative">${b.avg_max_drawdown != null ? b.avg_max_drawdown.toFixed(1)+'%' : '--'}</strong></div>
            </div>
            ${trapNote}${corrNote}
            <table class="basket-holdings">
                <thead><tr><th>Ticker</th><th>Real Yld</th><th>Net</th><th>Quality</th><th>Max DD</th><th>Div Cuts</th></tr></thead>
                <tbody>
                    ${data.holdings.map(h => `<tr>
                        <td><strong>${h.ticker}</strong>${h.is_trap ? ' ⚠' : ''}</td>
                        <td class="${h.real_yield>=0?'positive':'negative'}">${pct(h.real_yield)}</td>
                        <td class="${h.net_real_yield>=0?'positive':'negative'}">${pct(h.net_real_yield)}</td>
                        <td>${h.quality_score != null ? h.quality_score.toFixed(0) : '--'}</td>
                        <td class="negative">${h.max_drawdown != null ? h.max_drawdown.toFixed(1)+'%' : '--'}</td>
                        <td class="${h.div_cuts_12m>0?'negative':''}">${h.div_cuts_12m || 0}</td>
                    </tr>`).join('')}
                </tbody>
            </table>`;
    } catch (e) {
...
async function loadDistYield() {
    try {
        const etfs = await fetchEtfsWithMode('current_yield', 'desc', 1000);
...
async function loadDistCoverage() {
    try {
        const best = await fetchEtfsWithMode('distribution_coverage', 'desc', 1000);
        const worst = best.slice().reverse(); // re-sort worst on client from same data
...
async function loadNavErosion() {
    try {
        const all = await fetchEtfsWithMode('nav_annual_change', 'desc', 2000);
...
async function loadTotalReturn() {
    try {
        ...
        const all = await fetchEtfsWithMode('total_return_1yr', 'desc', 2000);
...
async function loadSharpe() {
    try {
        const all = await fetchEtfsWithMode(null, null, 2000);
...
async function loadBetaChart() {
    try {
        ...
        function renderPeriod(period) {
            ...
            const mode = getUniverseMode();
            const params = new URLSearchParams({ period: periodLabel, mode });
            if (mode === 'full') {
                const q = getQualityParams();
                for (const [k, v] of Object.entries(q)) params.set(k, v);
            }
            fetch(`${API}/beta-correlation?${params}`).then(r => r.json()).then(data => {
...
async function loadReturnChart() {
    try {
        const all = await fetchEtfsWithMode('total_return_3yr', 'desc', 2000);
...
async function setupPortfolio() {
    try {
        if (allEtfs.length === 0) allEtfs = await fetchEtfsWithMode(null, null, 2000);
...
async function loadBestPortfolios() {
    try {
        ...
        // Wire SMA controls
        ['bp-sma-period', 'bp-sma-direction'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.onchange = loadBestPortfolios;
        });
        // Wire checkboxes
        ...
        const results = await Promise.all(checked.map(async sortBy => {
            const mode = getUniverseMode();
            const params = new URLSearchParams({ period, sort_by: sortBy, mode });
            // SMA filter
            const smaPeriod = document.getElementById('bp-sma-period')?.value || '0';
            if (smaPeriod !== '0') {
                params.set('sma_period', smaPeriod);
                params.set('sma_direction', document.getElementById('bp-sma-direction')?.value || 'above');
            }
            if (mode === 'full') {
                const q = getQualityParams();
                for (const [k, v] of Object.entries(q)) params.set(k, v);
            }
            const d = await (await fetch(`${API}/best-portfolios?${params}`)).json();
